...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { orderId?: string; method?: 'card' | 'upi'; outcome?: 'authorised' | 'refused'; recoveredFromAttemptId?: string };
    ...
    const failureCategory = body.outcome === 'refused' ? 'payment_method_declined' : null;
    await db.batch([
